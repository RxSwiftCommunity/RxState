...
final class RootToTasksCoordinator: FlowCoordinatorType {
    struct Inputs: ActionCreatorInputsType {
        let store: StoreType
    }
    
    static func create(inputs: RootToTasksCoordinator.Inputs) -> Driver<ActionType> {
...
//        }
        ...
        return inputs.store.flowState
            ...
            .flatMap { (originRoute: Route?, destinationRoute: Route, navigatableController: NavigatableController) -> Observable<ActionType> in
                return RootToTasksCoordinator.navigate(fromRoute: originRoute, toRoute: destinationRoute, usingNavigatableController: navigatableController, inputs: inputs).asObservable()
            }
    ...
    private static func navigate(
        ...
        , inputs: RootToTasksCoordinator.Inputs
        ) -> Driver<ActionType> {
