...
final class ToRootCoordinator: FlowCoordinatorType {
    struct Inputs: ActionCreatorInputsType {
        let store: StoreType
        let window: UIWindow
    }
    
    static func create(inputs: ToRootCoordinator.Inputs) -> Driver<ActionType> {
        ...
        return inputs.store.flowState
            ...
            .flatMap { (originRoute: Route?, destinationRoute: Route, navigatableController: NavigatableController) -> Observable<ActionType> in
                return ToRootCoordinator.navigate(fromRoute: originRoute, toRoute: destinationRoute, usingNavigatableController: navigatableController, inputs: inputs).asObservable()
            }
    ...
    private static func navigate(
        ...
        , inputs: ToRootCoordinator.Inputs
        ) -> Driver<ActionType> {
