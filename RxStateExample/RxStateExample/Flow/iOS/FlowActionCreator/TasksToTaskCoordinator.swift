...
final class TasksToTaskCoordinator: FlowCoordinatorType {
    struct Inputs: ActionCreatorInputsType {
        let store: StoreType
        let taskId: TaskId
    }
    
    static func create(inputs: TasksToTaskCoordinator.Inputs) -> Driver<ActionType> {
        ...
        return inputs.store.flowState
            ...
            .flatMap { (originRoute: Route?, destinationRoute: Route, navigatableController: NavigatableController) -> Observable<ActionType> in
                return TasksToTaskCoordinator.navigate(fromRoute: originRoute, toRoute: destinationRoute, usingNavigatableController: navigatableController, inputs: inputs).asObservable()
            }
    ...
    private static func navigate(fromRoute originRoute: Route?, toRoute destinationRoute: Route, usingNavigatableController navigatableController: NavigatableController, inputs: TasksToTaskCoordinator.Inputs) -> Driver<ActionType> {
